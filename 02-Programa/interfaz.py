...
class VentanaPrincipal:
    ...
    def _configurar(self):
        ...
        botones = [
            ...
            ("Buscar por codigo", self._buscar),
            ...
            ("Busq. Lineal", self._busq_lineal),
            ("Busq. Binaria", self._busq_binaria),
        ]
    ...
    def _eliminar_producto(self):
        cod = simpledialog.askstring("Eliminar", "Codigo del producto:")
        if not cod: return
        prod = self._inventario.eliminar_producto(cod.strip())
        if prod:
            self._mostrar(f"OK Producto eliminado: {prod.nombre}")
        else:
            self._mostrar("Producto no encontrado")

    def _buscar(self):
        cod = simpledialog.askstring("Buscar", "Codigo:")
        if not cod: return
    ...
    def _eliminar_proveedor(self):
        ...
        if self._inventario.eliminar_proveedor(ruc.strip()):
            self._mostrar(f"OK Proveedor eliminado")
        else:
    ...
    def _rotar(self):
        ...
        self._mostrar("OK Inventario rotado (el primer producto ahora es el ultimo)")
    ...
    def _ord_burbuja(self):
        ...
        self._mostrar(f"=== ORDENADO POR {criterio} (Burbuja O(n2)) ===")
        for p in prods:
            self._mostrar(p.mostrar_info())
    ...
    def _busq_lineal(self):
        nom = simpledialog.askstring("Busqueda Lineal", "Nombre del producto:")
        if not nom: return
        idx, prod = self._inventario.buscar_lineal_por_nombre(nom.strip())
        if prod:
            self._mostrar(f"OK Busqueda lineal: encontrado en posicion {idx}")
            self._mostrar(prod.mostrar_info())
        else:
            self._mostrar("Producto no encontrado")

    def _busq_binaria(self):
        cod = simpledialog.askstring("Busqueda Binaria", "Codigo del producto:")
        if not cod: return
        idx, prod = self._inventario.buscar_binaria_por_codigo(cod.strip())
        if prod:
            self._mostrar(f"OK Busqueda binaria: encontrado en posicion {idx}")
            self._mostrar(prod.mostrar_info())
        else:
            self._mostrar("Producto no encontrado")
